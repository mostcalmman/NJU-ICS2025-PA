...
static void execute(uint64_t n) {
  ...
  for (;n > 0; n --) {
    ...
    if(total_exec_count > MAX_INSTR_LIMIT){
      Log("NEMU: "ANSI_FG_RED"Deadloop detected!"ANSI_FG_RED" Total instruction count exceeds %d", MAX_INSTR_LIMIT);
      nemu_state.state = NEMU_ABORT;
    }

    if (s.dnpc == s.pc) {
      Log("NEMU: "ANSI_FG_RED"Deadloop detected!"ANSI_FG_RED" Jmp in place at PC = " FMT_WORD, s.pc);
      nemu_state.state = NEMU_STOP;
    }
#endif
