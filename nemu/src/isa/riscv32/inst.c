...
#define immB() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 12) | (BITS(i, 30, 25) << 5) | (BITS(i, 11, 8) << 1) | BITS(i, 7, 7); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  */
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 0110011", sra    , R, R(rd) = src1 >> src2); // msb-extends
  ...
  INSTPAT("0100000?????  ????? 101 ????? 0010011", srai   , I, R(rd) = src1 >> imm); // msb-extends
  INSTPAT("????????????  ????? 010 ????? 0010011", slti   , I, R(rd) = (src1 < imm)?1:0);
  INSTPAT("????????????  ????? 011 ????? 0010011", sltiu  , I, R(rd) = (src1 < imm)?1:0); // zero-extends

  /* ------------------------- 读写 ------------------------- */
  INSTPAT("????????????  ????? 001 ????? 0000011", lb     , I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("????????????  ????? 010 ????? 0000011", lh     , I, R(rd) = Mr(src1 + imm, 2));
  INSTPAT("????????????  ????? 011 ????? 0000011", lw     , I, R(rd) = Mr(src1 + imm, 4));
  ...
  /* ------------------------- 分支与跳转 ------------------------- */
  INSTPAT("??????? ????? ????? 000 ????? 1100011", beq    , B, if(src1 == src2) s->dnpc = s->pc + (int32_t)imm);
  INSTPAT("??????? ????? ????? 001 ????? 1100011", bne    , B, if(src1 != src2) s->dnpc = s->pc + (int32_t)imm);
  INSTPAT("??????? ????? ????? 100 ????? 1100011", blt    , B, if(src1 <  src2) s->dnpc = s->pc + (int32_t)imm);
  INSTPAT("??????? ????? ????? 101 ????? 1100011", bge    , B, if(src1 >= src2) s->dnpc = s->pc + (int32_t)imm);
  INSTPAT("??????? ????? ????? 110 ????? 1100011", bltu   , B, if(src1 <  src2) s->dnpc = s->pc + (int32_t)imm);
  INSTPAT("??????? ????? ????? 111 ????? 1100011", bgeu   , B, if(src1 >= src2) s->dnpc = s->pc + (int32_t)imm);
