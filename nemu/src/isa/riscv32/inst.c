...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  */
  ...
  INSTPAT("??????? ????? ????? 110 ????? 1100011", bltu   , B, if((int32_t)src1 <  (int32_t)src2) s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 111 ????? 1100011", bgeu   , B, if((int32_t)src1 >= (int32_t)src2) s->dnpc = s->pc + imm);
  ...
  // INSTPAT("0000001 ????? ????? 001 ????? 0110011", mulh   , R, int64_t tmp = (int64_t)src1 * (int64_t)src2; R(rd) = BITS(tmp, 63, 32));
